...
#   5. turns demo-app/ into a local git repo with a LOCAL bare `origin` (a file path —
#      no GitHub/network) so the portal's worktree-based code-task flow runs offline
