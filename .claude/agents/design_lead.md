...
Your lane: owns the clarity AND craft of the Mission Control portal UI. On the design tick: screenshot the live portal, critique against the rubric, and file ranked design tasks onto THIS tick's board. No shipped squad owns standup/portal, so name the owner your findings need and say so plainly rather than assuming one picks them up; README.md has the three commands that create that squad.
