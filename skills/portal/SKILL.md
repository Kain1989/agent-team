...
3. **Tell the human the URL** (http://127.0.0.1:8770 or the `PORT` from `.env`) and what they'll see: squads + dev health, the EM board, the live tick log, and the **job queue** — submit a code task (target `project:demo-app` if a `demo-app/` exists; otherwise any developer's `folder` from `standup/team.json`), watch it run in an isolated worktree, review the diff, and **Approve** to commit.
