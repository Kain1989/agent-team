...
2. **You are the team lead.** Create a native agent team and **spawn one teammate per active role** using the generated agent types — read them out of `standup/team.json` rather than typing ids from memory. A fresh install ships only the staff roles (`pm_agent`, `design_lead`, `product_qa`; `comms_triage` is inactive) and no squads at all, so the developer ids are whatever `/add-project` or `/add-team` + `/add-role` created. Keep each teammate in its lane (the role's `focus`); honor the pairs — the two developers of a squad challenge each other's plan and diff in a fresh context.
